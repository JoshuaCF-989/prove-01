...
public class Lists
{
    ...
    public static void RotateListRight(List<int> data, int amount)
    {
        ...
        {
            int saveEvenValue = 0;
            int saveOddValue = data[0];
            int saveLastValue = data[data.Count - 1];
            for (int i = 0; i < data.Count; i++)
            {
                if (i == data.Count-1)
                {
                    data[0] = saveLastValue;
                }
                else if (i % 2 == 0)
                {
                    saveEvenValue = data[i + 1];
                    data[i + 1] = saveOddValue;
                }
                else if  (i % 2 == 1)
                {
                    saveOddValue = data[i + 1];
                    data[i + 1] = saveEvenValue;
                }
            }
            index++;
        }
